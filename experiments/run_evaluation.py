...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("results/phase1/baseline_evaluation.json"))
...
def expected_budget(method_name: str, candidate_pool_size: int) -> int:
    ...
    if method_name.startswith("fixed_prefix_") or method_name.startswith("random_"):
        try:
            ...
            raise ValueError(f"Method {method_name!r} does not end in an integer budget") from error
...
def validate_method_predictions(
    method_name: str, predictions: Sequence[Mapping[str, Any]], top_k: int, candidate_pool_size: int
) -> None:
    ...
    if not 0 <= budget <= candidate_pool_size:
        raise ValueError(f"Method {method_name!r} has invalid reranking budget {budget}")
    query_ids: set[str] = set()
    for prediction in predictions:
        if not {"query_id", "document_ids", "reranked_pairs"} <= prediction.keys():
            raise ValueError(f"Method {method_name!r} has an incomplete prediction")
        query_id = str(prediction["query_id"])
        if query_id in query_ids:
            raise ValueError(f"Method {method_name!r} contains duplicate query ID {query_id!r}")
        query_ids.add(query_id)
        document_ids = prediction["document_ids"]
        if not isinstance(document_ids, list) or len(document_ids) > top_k:
            raise ValueError(f"Method {method_name!r}, query {query_id!r} exceeds top_k={top_k}")
        if prediction["reranked_pairs"] != budget:
            raise ValueError(
                f"Method {method_name!r}, query {query_id!r} has reranked_pairs="
                f"{prediction['reranked_pairs']}; expected {budget}"
            )
...
def evaluate_method(
    ...
    validate_method_predictions(method_name, predictions, top_k, candidate_pool_size)
    matched_predictions = [prediction for prediction in predictions if str(prediction["query_id"]) in qrels]
    missing_query_ids = sorted({str(prediction["query_id"]) for prediction in predictions} - set(qrels))
    ...
    for prediction in matched_predictions:
        query_id = str(prediction["query_id"])
        ranked_ids = [str(document_id) for document_id in prediction["document_ids"]]
        relevance = qrels[query_id]
        ndcg_scores.append(ndcg_at_k(ranked_ids, relevance, k=top_k))
    ...
    return {
        ...
        "compression_ratio": sum(compression_ratio(candidate_pool_size, count) for count in pair_counts) / len(pair_counts)
        if pair_counts
        else 0.0,
    }
...
def print_table(results: Mapping[str, Mapping[str, Any]]) -> None:
    ...
    print("Method | nDCG@10 | Recall@10 | MRR@10 | Avg Pairs | Total Pairs | Compression")
    print("--- | ---: | ---: | ---: | ---: | ---: | ---:")
    for method_name, result in results.items():
        print(
            f"{method_name} | {result['ndcg_at_10']:.6f} | {result['recall_at_10']:.6f} | "
            f"{result['mrr_at_10']:.6f} | {result['average_reranked_pairs_per_query']:.2f} | "
            f"{result['total_reranking_pairs']} | {result['compression_ratio']:.2%}"
        )
...
def main() -> None:
    ...
    for method_name, method_data in prediction_data["methods"].items():
        ...
        if not isinstance(method_predictions, list):
            raise ValueError(f"Method {method_name!r} must contain a predictions list")
